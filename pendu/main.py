...
while continuer_partie != 'n':
    print("Joueur {}: {} point(s)".format(utilisateur, str(scores)))
    mot_a_trouver = f.choisir_mot()
    lettres_trouvees = []
    mot_trouve = f.recup_mot_masque(mot_a_trouver, lettres_trouvees)
    nb_chances = nb_coups
    while mot_a_trouver!=mot_trouve and nb_chances>0:
        print("Mot à trouver {} (encore {} chances)".format(mot_trouve, nb_chances))
        lettre = f.recup_lettre()
    ...
    if mot_a_trouver==mot_trouve:
        ...
        print("PENDU !!! Vous avez perdu. MOT: {}".format(mot_a_trouver))
...
print("Vous finissez la partie avec {} points.".format(str(scores)))